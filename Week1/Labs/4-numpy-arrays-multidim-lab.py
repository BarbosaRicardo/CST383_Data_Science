...
#%timeit [np.sum(x) for x in df]
#%timeit [sum(x) for x in df]
...
# The array will become a (1,35) and can be streched to fit (50,35)
...
#test = np.array([df_centered.mean(axis=0)])
