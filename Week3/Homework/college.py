...
# assume unemp is defined
np.sum(unemp < unemp.loc['COMPUTER SCIENCE']) / unemp.size
...
# Your result should be a single number.
sum(df['Total']) # I'm surprised sum is much faster than np.sum or pandas sum 13.7 microseconds vs 69.9 microseconds or 58.6 microseconds
...
df.index[df['ShareWomen'] == df['ShareWomen'].max()].format()[0]
...
df.loc[df['ShareWomen'] > 0.90, 'Median']
...
df.loc[df['ShareWomen'] < 0.15, 'Median']
...
df.loc[df['Rank'] == df['Rank'].min(), 'Median'].values[0] / df.loc[df['Rank'] == df['Rank'].max(), 'Median'].values[0]
...
df[['Median', 'P25th', 'P75th']].sort_values(by='Median', ascending=False).head(10)
...
df[['Median', 'P25th', 'P75th']].sort_values(by='Median', ascending=True).head(10)
